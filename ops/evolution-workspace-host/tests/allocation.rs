...
use maia_evolution_workspace_host::{Evidence, Gate, GitWorkspaceHost};
use std::fs;
...
impl Fixture {
    fn new() -> Self {
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let base = std::env::temp_dir().join(format!("maia-m0162-{}-{nonce}", std::process::id()));
    ...
    fn request(&self, id: &str) -> Request {
        let plan = Plan {
            ...
            proposed_paths: vec!["src/evolvable.rs".into()],

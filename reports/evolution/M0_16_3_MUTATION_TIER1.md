...
- GitHub Actions run `36131357797` verified the CRLF correction: the mutation and Tier-1 reached all checks, with `TargetedTests` the only failed check. The runner's fixed verifier clears the child environment, while MSVC discovery previously searched only `ProgramFiles(x86)` and dropped an explicitly configured `LIB` path. The verifier now discovers MSVC and Windows SDK locations under both `ProgramFiles` roots and preserves `LIB` in its restricted environment. Local targeted allocation tests, targeted clippy, default and all-features workspace tests, all 85 Python spec tests, spec validation/guard, formatting, diff check, and Round Table absence proof pass with this correction. Remote validation is pending.
